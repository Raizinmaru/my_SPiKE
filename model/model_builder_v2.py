...
from model import spike_v2

def create_model(config, num_coord_joints, backbone):
    """Create a model based on the config and return it."""
    if "ITOP" in config["dataset"]:
        try:
            model_name = config.get("model")
            if not hasattr(spike_v2, model_name):
                raise ValueError(f"Model {model_name} not found.")

            model_class = getattr(spike_v2, config["model"])
            model_params = {
