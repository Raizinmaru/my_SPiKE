...
class ConvPool(nn.Module):
    def __init__(self,
        ...
        if use_res:
            ...
            self.skipconv = create_convblock2d(skip_channel, channels[-1], norm_args=None, act_args=None,
    ...
    def forward(self, features):
        ...
        # fjの計算 (support_featuresをそのまま使用)
        #fj = features.permute(0, 3, 1, 2).contiguous() # (B, 3, 15, Nv)
        fj = features
...
class LocalAggregation(nn.Module):
    def __init__(self,
        ...
        if aggr_type.lower() == 'convpool':
            self.SA_CONFIG_operator = ConvPool(channels, conv_args, norm_args, act_args,
                                               #group_args, feature_type, 
                                               reduction, use_res, use_pooled_as_identity)
        elif aggr_type.lower() == 'assa':
            self.SA_CONFIG_operator = ASSA(channels, conv_args, norm_args, act_args,
                                           #group_args, feature_type, 
                                           reduction, use_res, use_inverted_dims)
    ...
    def forward(self, support_features):
        ...
        return self.SA_CONFIG_operator(support_features)
