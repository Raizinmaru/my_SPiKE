...
class DGCNN(nn.Module):
    def __init__(self, args):
        ...
        self.linear3 = nn.Linear(256, args.num_class)

    def forward(self, x):
        batch_size = x.size(0)

        # First edge conv
        x = get_graph_feature(x, k=self.k, use_normals=self.args.use_normals)  # (batch_size, 3, num_points) -> (batch_size, 3*2, num_points, k)
